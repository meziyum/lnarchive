...
function nested_comments() { //Function to print nested comment entry
    ...
    $author_url = get_comment_author_url(); //Get the Author URL
    ?>
...
function novel_reviews() { //Function to print novel reviews

    $gravatar = get_avatar( get_comment_author_email()); //Get the Avatar
    $author_url = get_comment_author_url(); //Get the Author URL
    ?>
 
    <li class="row review-entry" id="review-<?php comment_ID() ?>"> <!-- Review Entry -->

        <div class="review-header col-3 col-sm-2 col-md-2 col-lg-1"> <!-- Review Header -->
            <a href="<?php echo esc_url($author_url);?>"><?php echo $gravatar?></a> <!-- Display Avatar -->
        </div>

        <div class="review-body col"> <!-- Review Body -->
            <h4><a href="<?php echo esc_url($author_url);?>">
                <?php echo ucfirst(esc_html(get_comment_author()));?>
            </a></h4> <!-- Review Author -->
            <p><?php echo get_comment_text();?></p> <!-- Review Content -->
        </div>
    </li>
<?php
}
